...
激活和 attention 随 latent token 数增长，显存/计算压力主要由 `width * height` 决定。网关现在强制宽高为 8 的倍数、单边不超过 2048、总像素不超过 4,194,304，并限制 steps。工作站实测 1536² 和 2048² 的 4-step 任务均可完成；2048²、25 steps 实测约 96 秒且四张 V100 同时有负载。生产任务仍应先用低步数确认显存，再提高到 25–50 steps。超过 2048² 或 4MP 时应采用“基础图 + latent/像素分块放大”的两阶段流程，避免直接全局 attention。
